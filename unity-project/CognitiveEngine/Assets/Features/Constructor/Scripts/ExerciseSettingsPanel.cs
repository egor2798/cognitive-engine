...
public class ExerciseSettingsPanel : MonoBehaviour
{
    ...
    [SerializeField] private BodyPointBindingsPanel bodyPointBindingsPanel;
    ...
    [SerializeField] private GameObject bodyPointsTab;
    ...
    [SerializeField] private Button bodyPointsTabButton;
    ...
    [Header("Audio")]
    [SerializeField] private Toggle useAudioToggle;
    [SerializeField] private TMP_InputField audioVolumeInput;
    [SerializeField] private Toggle useMetronomeToggle;
    [SerializeField] private TMP_InputField metronomeBpmInput;
    [SerializeField] private TMP_InputField metronomeVolumeInput;
    ...
    public void ShowBodyPointsTab() => ShowTab(SettingsTab.BodyPoints);
    public void ShowAudioTab() => ShowTab(SettingsTab.Audio);
    ...
    private void ShowTab(SettingsTab tab)
    {
        ...
        if (bodyPointsTab != null) bodyPointsTab.SetActive(tab == SettingsTab.BodyPoints);
        if (audioTab != null) audioTab.SetActive(tab == SettingsTab.Audio);
    ...
    private void UpdateTabButtons()
    {
        ...
        if (bodyPointsTabButton != null) bodyPointsTabButton.interactable = currentTab != SettingsTab.BodyPoints;
        if (audioTabButton != null) audioTabButton.interactable = currentTab != SettingsTab.Audio;
    }
    ...
    public void LoadFromCurrentExercise()
    {
        ...
        SetToggle(useAudioToggle, settings.audioEnabled);
        SetToggle(useMetronomeToggle, settings.metronomeEnabled);
        ...
        SetInput(audioVolumeInput, Mathf.Clamp01(settings.audioVolume01) * 100f);
        SetInput(metronomeBpmInput, settings.metronomeBpm);
        SetInput(metronomeVolumeInput, Mathf.Clamp01(settings.metronomeVolume01) * 100f);
        ...
        if (bodyPointBindingsPanel != null)
        {
            if (settings.bodyPointBindings == null)
                settings.bodyPointBindings = new List<BodyPointBindingData>();

            bodyPointBindingsPanel.SetBindings(settings.bodyPointBindings);
        }
    }
    ...
    public void ApplyToCurrentExercise()
    {
        ...
        if (useAudioToggle != null) settings.audioEnabled = useAudioToggle.isOn;
        if (useMetronomeToggle != null) settings.metronomeEnabled = useMetronomeToggle.isOn;
        ...
        if (TryGetFloat(audioVolumeInput, out float audioVolumePercent)) settings.audioVolume01 = Mathf.Clamp01(audioVolumePercent / 100f);
        if (TryGetFloat(metronomeBpmInput, out float metronomeBpm)) settings.metronomeBpm = Mathf.Clamp(metronomeBpm, 20f, 240f);
        if (TryGetFloat(metronomeVolumeInput, out float metronomeVolumePercent)) settings.metronomeVolume01 = Mathf.Clamp01(metronomeVolumePercent / 100f);
        ...
        ApplyBodyPointsToSettings(settings);

        if (trackEditor != null)
            trackEditor.ApplyVisualSettingsFromCurrentExercise();

        int bodyPointCount = settings.bodyPointBindings != null ? settings.bodyPointBindings.Count : 0;

        Debug.Log($"Exercise settings applied: duration={settings.durationMs}ms, speed={settings.defaultSpeed}, input={settings.inputSource}, corridor={settings.corridorWidthCm}cm, hitRadius={settings.hitRadiusCm}cm, thickness={settings.lineThickness}, showTrack={settings.showTrack}, bodyPoints={bodyPointCount}, audio={settings.audioEnabled}, audioVolume={settings.audioVolume01}, metronome={settings.metronomeEnabled}, bpm={settings.metronomeBpm}");
    }

    private void ApplyBodyPointsToSettings(ExerciseSettingsData settings)
    {
        if (settings == null || bodyPointBindingsPanel == null)
            return;

        List<BodyPointBindingData> bindings = bodyPointBindingsPanel.GetBindingsCopy();
        settings.bodyPointBindings = bindings;

        BodyPointBindingData primary = bodyPointBindingsPanel.GetPrimaryBinding();

        if (primary != null)
        {
            settings.activeBodyPointId = primary.bodyPointId;
            settings.activeBodyPointZoneId = primary.bodyPointZoneId;
        }
        else if (bindings != null && bindings.Count > 0 && bindings[0] != null)
        {
            settings.activeBodyPointId = bindings[0].bodyPointId;
            settings.activeBodyPointZoneId = bindings[0].bodyPointZoneId;
        }
    }
