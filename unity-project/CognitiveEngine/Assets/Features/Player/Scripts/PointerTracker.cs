...
public class PointerTracker : MonoBehaviour
{
    ...
    [Header("Sensor Mode Startup Override")]
    [Tooltip("For today's demo: force sensor mode on scene start. Disable this if you want the Inspector/loaded settings to decide the mode.")]
    [SerializeField] private bool forceSensorControlModeOnStart = true;

    [Tooltip("Mode that will be applied in Awake/OnEnable when Force Sensor Control Mode On Start is enabled.")]
    [SerializeField] private SensorControlMode startupSensorControlMode = SensorControlMode.TiltAngles;

    [SerializeField] private bool logSensorControlModeOnStart = true;
    ...
    private void ApplyStartupSensorControlMode()
    {
        if (!forceSensorControlModeOnStart)
            return;

        sensorControlMode = startupSensorControlMode;

        if (logSensorControlModeOnStart)
            Debug.Log("[PointerTracker] Sensor control mode forced on start: " + sensorControlMode);
    }

    public void SetSensorControlMode(SensorControlMode mode)
    {
        sensorControlMode = mode;
        ResetSensorMotionStateForModeSwitch();
        Debug.Log("[PointerTracker] Sensor control mode set: " + sensorControlMode);
    }

    public void UseTiltAnglesMode()
    {
        SetSensorControlMode(SensorControlMode.TiltAngles);
    }

    public void UseImuDisplacementMode()
    {
        SetSensorControlMode(SensorControlMode.ImuDisplacement);
    }

    private void ResetSensorMotionStateForModeSwitch()
    {
